...
function normalizeAnime(raw: AniListMedia | null | undefined): AnimeRecord | null {
  ...
  const identity = identityFromProvider('anilist', String(raw.id), { malId: raw.idMal, anilistId: raw.id });
  return {
    externalId: raw.idMal != null ? String(raw.idMal) : externalId(raw.id),
    ...identity,
    title,
    titleAr: null,
...
export class AniListProvider implements AnimeProvider {
  ...
  async details(externalIdValue: string): Promise<AnimeRecord | null> {
    const raw = Number.parseInt(externalIdValue, 10);
    const isAniListId = Number.isInteger(raw) && raw < 0;
    const id = isAniListId ? Math.abs(raw) : raw;
    if (!Number.isInteger(id) || id <= 0) throw new Error('Not a supported AniList external id');

    const response = await this.query(`
      query ($id: Int, $idMal: Int) {
        Media(${isAniListId ? 'id: $id' : 'idMal: $idMal'}, type: ANIME) {
          id
          idMal
          title { english romaji native }
          description(asHtml: false)
