...
async function manga(page: number, limit: number): Promise<ContentPage> {
  ...
  // MangaDex's documented manga ordering uses fields such as followedCount
  // and latestUploadedChapter. Keep the request conservative so the public
  // catalog endpoint works reliably from Cloudflare Workers.
  ...
  const json = await fetchJson(`https://api.mangadex.org/manga?${params.toString()}`);
