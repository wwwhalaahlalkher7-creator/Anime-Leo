import { isCompletedStatus, upsertAnimeBatch } from './catalog';
  ...
  for (const stream of streams) {
    ...
    for (let i = 0; i < pagesPerStream; i += 1) {
      try {
        ...
        const completedItems = result.items.filter((item) => isCompletedStatus(item.status));
        if (completedItems.length) {
          await upsertAnimeBatch(db, completedItems);
          upserted += completedItems.length;
        }
