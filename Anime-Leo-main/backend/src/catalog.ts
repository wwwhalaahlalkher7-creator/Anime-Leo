...
export async function searchCatalog(
  ...
  if (cached.items.length >= limit || cached.hasNext) {
    return {
      items: cached.items,
      page,
  ...
  try {
    ...
    const seen = new Set(cached.items.map((item) => item.externalId));
    await upsertAnimeBatch(
      db,
      providerPage.items.filter((item) => !seen.has(item.externalId)),
    );

    return {
      items,
      page,
      hasNextPage: providerPage.hasNextPage || providerPage.items.length > limit,
      source: cached.items.length ? 'database+provider' : 'provider',
      degraded: false,
    };
  } catch (_) {
    // Provider outage must not turn a healthy API/D1 into a 500.
    // Return whatever stale catalog data exists, or an empty but valid result.
    return {
      items: cached.items,
      page,
      hasNextPage: cached.hasNext,
...
export async function getAnime(
  ...
  try {
    row = await db.prepare('SELECT * FROM anime WHERE external_id = ? LIMIT 1').bind(externalId).first<Record<string, unknown>>();
  } catch (error) {
  ...
  try {
    ...
    if (item) await upsertAnime(db, item);
...
export async function getEpisodes(
  ...
  const resultRows = rows.results ?? [];
  const dbItems = resultRows.slice(0, limit).map(rowToEpisode);
  const dbHasNext = resultRows.length > limit;
  ...
  if (dbItems.length >= limit || dbHasNext) {
    return {
  ...
  try {
    ...
    await upsertEpisodes(db, providerPage.items);
...
export function animeToApi(item: AnimeRecord) {
  return {
    id: item.externalId,
    mal_id: Number(item.externalId) || item.externalId,
    title: item.title,
